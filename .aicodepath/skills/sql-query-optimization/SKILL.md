...
description: Investigate slow queries, N+1 problems, and missing indexes — EXPLAIN analysis, index strategy, and rewriting.
user-invocable: true
allowed-tools: Read, Bash, Glob, Grep
argument-hint: "<slow query or performance issue description>"
---
...
| N+1 queries in loops | Network latency x N | Use JOIN or batch loading |
...
```sql
-- Increase work_mem for complex queries (reloadable)
SET work_mem = '256MB';

-- Increase shared_buffers for better caching (25% of RAM)
-- Requires full PostgreSQL server restart
ALTER SYSTEM SET shared_buffers = '8GB';

-- Enable auto-vacuum
ALTER SYSTEM SET autovacuum = on;

-- Reload config (not for shared_buffers)
...
- Load `references/explain-analysis.md` when: Reading EXPLAIN output, understanding query plans, analyzing buffer statistics
- Load `references/performance-monitoring.md` when: Setting up monitoring, tracking slow queries over time, monitoring cache hit ratios

**Index Optimization**:
- Load `references/index-strategies.md` when: Choosing index type, creating composite indexes, using covering indexes, implementing partial indexes

**Query Optimization**:
- Load `references/query-rewrites.md` when: Rewriting slow queries, converting subqueries to JOINs, eliminating N+1 queries, implementing pagination

**Systematic Process**:
- Load `references/optimization-workflow.md` when: Following step-by-step optimization process, measuring improvements

**Error Resolution**:
- Load `references/error-catalog.md` when: Debugging specific errors, understanding root causes
...
```bash
templates/explain-query.sql      # EXPLAIN query templates
templates/index-examples.sql     # Index creation patterns
templates/query-rewrites.sql     # Query rewrite examples
templates/monitoring-queries.sql # Monitoring queries
```
