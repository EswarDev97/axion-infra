...
A comprehensive skill for creating optimized, agent-executable slash commands in AICodePath projects. This skill guides you through the entire process of designing, implementing, and testing reusable workflow automation commands.
...
The AICodePath Command Creator skill helps you transform repetitive workflows into reusable slash commands that can be invoked with `/command-name` in Claude Code conversations. It provides expert guidance on command structure, agent optimization, and best practices to ensure your commands execute reliably and autonomously.

**Purpose**: Create high-quality, agent-executable slash commands with proper structure, clear instructions, and optimal tool usage patterns for AICodePath projects.
...
Slash commands are markdown files stored in `.aicodepath/commands/` (project-level) or `~/.aicodepath/commands/` (global/user-level) that get expanded into prompts when invoked.
...
**Storage Locations**:
- **Project-level**: `.aicodepath/commands/my-command.md` (only available in this project)
- **Global/User-level**: `~/.aicodepath/commands/my-command.md` (available everywhere)
...
The AICodePath Command Creator follows a structured 6-step workflow:

### Step 1: Determine Location

**Auto-detection Logic**:
1. Check if current directory is inside a git repository
2. Default to project-level (`.aicodepath/commands/`) if in git repo
3. Default to global (`~/.aicodepath/commands/`) if not in git repo
...
### Project-Level Commands (`.aicodepath/commands/`)
...
### Global Commands (`~/.aicodepath/commands/`)
...
4. Generates command with Bash tool
5. Creates `.aicodepath/commands/ensure-ci.md`
6. User invokes: `/ensure-ci`

### Example 2: Create Global Code Review Command

**User Request**: "Create a global command to review code"
...
3. Gathers info:
   - Name: `code-review`
   - Description: "Review code files using AICodePath standards"
   - Arguments: `<files>` (required)
   - Steps: Check standards → Review files → Present results
4. Generates command with file analysis
5. Creates `~/.aicodepath/commands/code-review.md`
6. User invokes: `/code-review src/app.ts src/utils.ts`
...
4. Generates command with git analysis
5. Creates `.aicodepath/commands/submit-stack.md`
...
**Use Task tool for**:
- Specialized agents (planning, exploration)
...
The AICodePath Command Creator skill provides a comprehensive, guided workflow for creating high-quality slash commands in AICodePath projects. By following proven patterns, gathering detailed requirements, and generating agent-optimized instructions, it ensures your commands are:
...
2. Invoke the aicodepath-command-creator skill
