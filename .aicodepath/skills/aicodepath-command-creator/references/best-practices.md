...
- "Read src/config.ts lines 45-67 to understand the config structure"
- "Use Edit tool to replace 'string[]' with 'Array<string>'"
...
- "After running npm run format, all files should be formatted"
...
- "If error shows: `src/auth/login.ts:45: Type error`"

❌ WRONG:

- "Example: `git commit -m 'foo bar'`"
- "Example: `/submit-stack 'something'`"
- "If error shows: `file.ts:123: Error message`"
```
...
```markdown
**Use the Bash tool for pytest/pyright/ruff/prettier/make/npm/git commands:**
...
- **DO NOT batch completions** - mark tasks complete immediately after finishing
...
**SUCCESS**: Stop when all checks pass (exit code 0)
...
- Create tasks at the start for each iteration
...
Mark tasks as completed IMMEDIATELY after finishing each task, not batched at the end.
```
...
### Pattern: AICodePath Context Gathering
...
1. **AICodePath files** (if they exist):
   - AICODEPATH.md - project guidelines
   - aicodepath-docs/docs/ - documentation
...
- [ ] File path is correct (`.aicodepath/commands/` or `~/.aicodepath/commands/`)
...
- Run auto-fix command for the specific linter
- For manual fixes, use Edit tool to modify files
- Verify fixes by running linter again
```
...
```markdown
Fix all the errors, then mark all tasks as completed
```
...
2. Mark the task as completed immediately
3. Move to next category

Do NOT batch task completions at the end
```
...
Follow project's PR submission workflow
```
...
## AICodePath-Specific Best Practices

### Context File Priority for AICodePath

```markdown
Check these files in priority order:

1. **AICodePath core files**:
   - AICODEPATH.md - Project guidelines and structure
   - aicodepath-docs/docs/ - Project-specific documentation
   - .PLAN.md - Current implementation plan

2. **Standard context files**:
...
### Integration with AICodePath Hooks

```markdown
### Hook Integration

Before executing actions, check for relevant hooks:

\`\`\`bash
# Check if pre-commit hook exists
if [ -f .aicodepath/hooks/pre-commit ]; then
  # Consider hook requirements in workflow
fi
\`\`\`
...
1. Verify all filenames use kebab-case
2. Check for underscores or camelCase
3. Report violations to user
4. Suggest correct naming: `user_service.ts` → `user-service.ts`
```
