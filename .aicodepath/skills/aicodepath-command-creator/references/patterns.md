...
1. Check for context files (e.g., `.PLAN.md`, `AICODEPATH.md`)
...
4. Submit PRs with git workflow
...
1. Run check command (e.g., `make all-ci`, `npm test`)
...
2. If check fails:
   - Parse error output by category (linting, type errors, tests)
   - Create tasks for each error category
   - Apply fixes for each category sequentially
   - Mark task complete after fixing each category
...
4. After user approves plan
   - Save to .PLAN.md or appropriate location
...
```markdown
1. Use Task tool with Explore agent to find relevant files
   - Search for specific patterns
   - Identify key components

2. Use Task tool with Planning agent to create plan
...
3. Execute the plan directly in the main conversation
   ...
   - Use TaskCreate to track phases
...
2. AICODEPATH.md - AICodePath project guidelines
...
```markdown
**IMPORTANT:** Always use Bash tool for pytest/pyright/ruff/prettier/make commands

1. Use Bash tool directly
   - Run commands like: "make all-ci", "pytest tests/", "npm test", etc.
...
2. If issues found:
   ...
   - Add tasks for each issue category
...
- Makefile Integration (use make commands)
- Progressive Disclosure (expand tasks as issues found)
