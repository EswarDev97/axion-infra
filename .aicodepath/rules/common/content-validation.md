...
### JSON Validation
```markdown
## BEFORE creating any file with JSON:

1. Validate JSON syntax (proper quotes, commas, brackets)
...
### YAML Validation
```markdown
## BEFORE creating any file with YAML:

1. Validate YAML indentation (consistent spaces, no tabs)
2. Ensure proper quoting for special characters
3. Validate list and map structures
...
- **TypeScript/JavaScript**: Validate syntax, check for missing semicolons if required
- **Python**: Validate indentation consistency
- **SQL**: Validate query syntax basics
- **Shell/Bash**: Check for proper quoting

### Best Practices
1. Always specify language in code blocks (```typescript, ```python, etc.)
2. Ensure code is properly indented
3. Close all brackets, braces, and parentheses
4. Escape backticks within code blocks if needed
