"""Build and restart CRM-affected containers on the Mindflow server."""

import io, base64, struct, sys
import paramiko
from cryptography.hazmat.primitives import serialization
from cryptography.hazmat.primitives.asymmetric import rsa
from cryptography.hazmat.backends import default_backend

PPK_PATH = r"D:\Mindflow Project\Milesweb-Proxy-Server.ppk"
HOST = "103.108.220.42"; PORT = 22; USERNAME = "root"

def _read_mpint(buf):
    (l,) = struct.unpack(">I", buf.read(4)); return int.from_bytes(buf.read(l), "big", signed=True)
def _read_string(buf):
    (l,) = struct.unpack(">I", buf.read(4)); return buf.read(l)
def load_ppk(path):
    with open(path) as f: lines = [l.rstrip("\r\n") for l in f]
    header, pub64, priv64 = {}, [], []
    i = 0
    while i < len(lines):
        line = lines[i]
        if ":" in line and not line.startswith(" "):
            k, _, v = line.partition(":"); k, v = k.strip(), v.strip()
            if k == "Public-Lines":  n=int(v); pub64=lines[i+1:i+1+n];  i+=1+n; continue
            if k == "Private-Lines": n=int(v); priv64=lines[i+1:i+1+n]; i+=1+n; continue
            header[k] = v
        i += 1
    pb = io.BytesIO(base64.b64decode("".join(pub64))); _read_string(pb)
    e = _read_mpint(pb); n = _read_mpint(pb)
    sb = io.BytesIO(base64.b64decode("".join(priv64)))
    d=_read_mpint(sb); p=_read_mpint(sb); q=_read_mpint(sb); iqmp=_read_mpint(sb)
    priv = rsa.RSAPrivateNumbers(p,q,d,rsa.rsa_crt_dmp1(d,p),rsa.rsa_crt_dmq1(d,q),iqmp,
                                  rsa.RSAPublicNumbers(e,n)).private_key(default_backend())
    pem = priv.private_bytes(serialization.Encoding.PEM, serialization.PrivateFormat.OpenSSH, serialization.NoEncryption())
    return paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))

def run(client, cmd, label="", timeout=600):
    if label: print(f"\n--- {label} ---")
    print(f"$ {cmd[:100]}{'...' if len(cmd)>100 else ''}")
    _, stdout, stderr = client.exec_command(cmd, timeout=timeout)
    # Stream stdout line by line
    while True:
        line = stdout.readline()
        if not line: break
        safe = line.rstrip().encode("ascii", errors="replace").decode("ascii")
        print(safe)
    err = stderr.read().decode("utf-8", errors="replace")
    rc = stdout.channel.recv_exit_status()
    if err:
        safe_err = err.rstrip().encode("ascii", errors="replace").decode("ascii")
        print("[stderr]", safe_err[:500])
    print(f"[exit={rc}]")
    return rc

def main():
    pkey = load_ppk(PPK_PATH)
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(hostname=HOST, port=PORT, username=USERNAME, pkey=pkey,
                   timeout=30, allow_agent=False, look_for_keys=False)
    print("Connected.\n")

    # Build hr-service
    print("=== Building hr-service ===")
    rc = run(client, "cd /opt/mindflow && docker compose build hr-service 2>&1",
             "Build hr-service", timeout=600)
    if rc != 0:
        print("hr-service build FAILED"); client.close(); return 1

    # Build api-gateway
    print("\n=== Building api-gateway ===")
    rc = run(client, "cd /opt/mindflow && docker compose build api-gateway 2>&1",
             "Build api-gateway", timeout=600)
    if rc != 0:
        print("api-gateway build FAILED"); client.close(); return 1

    # Build frontend
    print("\n=== Building frontend ===")
    rc = run(client, "cd /opt/mindflow && docker compose build frontend 2>&1",
             "Build frontend", timeout=900)
    if rc != 0:
        print("frontend build FAILED"); client.close(); return 1

    # Restart all three
    print("\n=== Restarting containers ===")
    rc = run(client,
             "cd /opt/mindflow && docker compose up -d hr-service api-gateway frontend 2>&1",
             "docker compose up -d", timeout=120)
    if rc != 0:
        print("Restart FAILED"); client.close(); return 1

    # Wait and health check
    import time; time.sleep(15)
    run(client,
        "docker ps --filter 'name=axionpcs-hr-service' --filter 'name=axionpcs-api-gateway' "
        "--filter 'name=axionpcs-frontend' "
        "--format 'table {{.Names}}\\t{{.Status}}\\t{{.Ports}}'",
        "Container status")

    run(client, "docker exec axionpcs-hr-service curl -sf http://localhost:8102/health",
        "HR service health")
    run(client, "docker exec axionpcs-api-gateway curl -sf http://localhost:3001/health",
        "API Gateway health")

    client.close()
    print("\n=== DEPLOYMENT COMPLETE ===")
    print("CRM live at: https://mindflow.axionpcs.in/dashboard/crm")
    return 0

if __name__ == "__main__":
    sys.exit(main())
