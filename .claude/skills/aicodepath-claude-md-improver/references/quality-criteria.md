...
2. Cross-reference with actual codebase:
   - Verify documented commands exist (check package.json, Makefile, scripts/)
   - Check if referenced files/directories still exist
   - Verify architecture descriptions match current directory structure
...
## AICodePath-Specific Criteria

When assessing an AICodePath project, apply these additional checks on top of the standard rubric:

### CLAUDE.md (root)
- Does it reference the `.aicodepath/` directory structure?
- Are the Quick Commands (`init`, `/aicodepath-diagnostics`, `/aicodepath-status`) documented?
- Does it describe the AIDLC phases (PRE-FLIGHT → INCEPTION → CONSTRUCTION → OPERATIONS)?
- Does it reference where key docs live (`docs/`, `hooks/`, `skills/`, `agents/`)?

### .aicodepath/CLAUDE.md
- Are the Critical Development Rules present (file placement, path-resolver, logger, valid hook output)?
- Is the directory structure table accurate and complete?
- Are Key Files up to date?
- Does it describe the process for adding hooks, skills, and agents?

### .aicodepath/DEVELOPER-GUIDE.md
- Are platform requirements (Node.js 18+, python3) documented?
- Are the First Steps commands accurate?
- Does the Quick Task Reference cover hooks, skills, agents, guidelines, and DB migrations?
- Is the Pre-Commit Checklist complete and accurate?
...
- Duplicate info across multiple CLAUDE.md files in a monorepo
- AICodePath project: `appendToSystemPrompt` mentioned (deprecated — silently ignored)
- AICodePath project: hardcoded paths instead of `pathResolver` references
- AICodePath project: `console.log` or `console.error` instead of `logger` references
