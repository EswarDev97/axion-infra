...
Searches for handoff documents in aicodepath-docs/handoffs/ and displays:
- Filename with date
...
def get_project_root() -> str:
    ...
    os.getcwd() is unreliable — if this script is invoked from inside
    .aicodepath/, the handoff lookup silently targets the wrong directory.
    """
    try:
...
def main():
    # Get project path
    project_path = sys.argv[1] if len(sys.argv) > 1 else get_project_root()

    handoffs = list_handoffs(project_path)

    if not handoffs:
        print(f"No handoffs found in {project_path}/aicodepath-docs/handoffs/")
        print("\nTo create a handoff, run: python create_handoff.py [task-slug]")
        return

    print(f"Found {len(handoffs)} handoff(s) in {project_path}/aicodepath-docs/handoffs/\n")
    print("-" * 80)
