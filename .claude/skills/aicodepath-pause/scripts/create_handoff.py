...
def find_previous_handoffs(project_path: str) -> list[dict]:
    ...
    handoffs_dir = Path(project_path) / "aicodepath-docs" / "handoffs"
...
def build_aidlc_state_block(aidlc_state: dict) -> str:
    """Build the ## AIDLC Workflow State section from a structured dict."""
    phase = aidlc_state.get("phase", "CONSTRUCTION")
    next_skill = aidlc_state.get("next_skill", "")
    batch = aidlc_state.get("batch", "")
    task = aidlc_state.get("task", "")
    task_title = aidlc_state.get("task_title", "")
    plan_file = aidlc_state.get("plan_file", "")

    return f"""## AIDLC Workflow State
...
1. Invoke `{next_skill}` to continue from where this session left off.
2. Review the plan file at `{plan_file}` for remaining tasks.
3. Check the active task file in `aicodepath-docs/task/` for current task status.
"""
...
"""
    ...
    # Append AIDLC Workflow State block if provided, then close with footer
    if aidlc_state:
        content += "\n" + build_aidlc_state_block(aidlc_state)
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "JSON string with AIDLC workflow state fields: "
            "phase, next_skill, batch, task, task_title, plan_file"
        ),
    )

    args = parser.parse_args()

    # Get project path — use git root to be cwd-independent.
    # os.getcwd() is unreliable: if the script is invoked from inside .aicodepath/,
    # handoffs land in .aicodepath/aicodepath-docs/handoffs/ (wrong path).
    success, git_root = run_cmd(["git", "rev-parse", "--show-toplevel"])
    if success and git_root:
        project_path = git_root
    else:
        # Fallback: walk up from script location
        # (.aicodepath/skills/aicodepath-pause/scripts/ → 4 parents → project root)
    ...
    # Parse --aidlc-state JSON if provided
    aidlc_state = None
    if args.aidlc_state:
        import json as _json
        aidlc_state = _json.loads(args.aidlc_state)

    # Generate handoff
    filepath = generate_handoff(project_path, args.slug, args.continues_from, aidlc_state)
    ...
    print(f"4. Run: python3 .aicodepath/skills/aicodepath-pause/scripts/validate_handoff.py {filepath}")
