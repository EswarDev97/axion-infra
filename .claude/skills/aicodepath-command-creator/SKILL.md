---
name: aicodepath-command-creator
description: Create AICodePath slash commands — optimized, agent-executable with proper structure and best practices.
version: 2.1.0
tags: [command-creation, automation, workflow, developer-tools]
user-invocable: true
...
This skill guides the creation of AICodePath slash commands - reusable workflows that can be invoked with `/command-name` in Claude Code conversations.

## About Slash Commands

Slash commands are markdown files stored in `.aicodepath/commands/` (project-level) or `~/.aicodepath/commands/` (global/user-level) that get expanded into prompts when invoked. They're ideal for:
...
2. Default location:
   - If in git repo → Project-level: `.aicodepath/commands/`
   - If not in git repo → Global: `~/.aicodepath/commands/`
3. Allow user override:
   - If user explicitly mentions "global" or "user-level" → Use `~/.aicodepath/commands/`
   - If user explicitly mentions "project" or "project-level" → Use `.aicodepath/commands/`
...
1. Determine full file path:
   - Project: `.aicodepath/commands/[command-name].md`
   - Global: `~/.aicodepath/commands/[command-name].md`
...
- Use Bash tool for `pytest`, `pyright`, `ruff`, `prettier`, `make`, `npm`, `git` commands
- Use Task tool to invoke subagents for specialized tasks
- Check for specific files first (e.g., `.PLAN.md`, `AICODEPATH.md`) before proceeding
- Mark todos complete immediately, not in batches
- Include explicit error handling instructions
- Define clear success criteria

## AICodePath-Specific Considerations

When creating commands for AICodePath projects:

- Check for `.aicodepath/` directory to confirm AICodePath initialization
- Reference `AICODEPATH.md` for project-specific guidelines and structure
- Use `aicodepath-docs/docs/` for documentation context
- Consider integration with AICodePath hooks in `.aicodepath/hooks/`
- Follow AICodePath naming conventions (kebab-case for all filenames)

## NEVER

- **NEVER** use underscores in command names — `submit_stack.md` is invoked as `/submit_stack`, but Claude Code's command parser treats underscores inconsistently across platforms. Kebab-case (`submit-stack.md` → `/submit-stack`) is the only safe convention. One wrong character and the command silently fails to resolve.
- **NEVER** write commands that assume a specific working directory — commands are invoked from wherever the user happens to be in the conversation. A command that runs `cat .PLAN.md` will fail if the user isn't at the project root. Always `cd` explicitly or use path detection at the start of the command.
- **NEVER** put sensitive values (API keys, credentials, project IDs) directly in the command file — command files live in version control. Parameterize secrets via environment variables or prompt the agent to ask the user at invocation time.
- **NEVER** write commands that do too much in one invocation without checkpoints — commands that execute 8 steps atomically create a hidden failure mode: step 7 fails, but steps 1-6 already ran. Break long workflows into stages with confirmation points, or design idempotent steps that can safely re-run.
- **NEVER** forget to include error handling instructions for the most likely failure points — an agent following a command without error guidance will either silently proceed on failure or stop with no recovery path. For each key step, specify what to do if it fails.
